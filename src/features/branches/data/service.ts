...
import type { UpsertBranchInput } from "@/features/branches/validators";
import type { BranchListItem, BranchOverview } from "@/features/branches/domain/entity";

export async function getBranchOverview(params: { tenantId: string }): Promise<BranchOverview> {
...
export async function listBranches(params: { tenantId: string; q?: string | null; categoryId?: string | null }) {
  ...
  const items = await prisma.branch.findMany({
    ...
    take: 200,
    select: { id: true, name: true, code: true, categoryId: true, phone: true, address: true, isActive: true, updatedAt: true, category: { select: { id: true, name: true } } },
  });
