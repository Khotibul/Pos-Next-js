...
export async function redeemLicense(params: { tenantId: string; serial: string }) {
  ...
  const license = await prisma.licenseKey.findUnique({
    where: { serial },
    select: { id: true, serial: true, planId: true, tenantId: true, expiresAt: true, revokedAt: true, redeemedAt: true },
  });
  ...
  await prisma.$transaction([
    prisma.licenseKey.update({ where: { id: license.id }, data: { tenantId: params.tenantId, redeemedAt: new Date() } }),
    prisma.tenant.update({
      where: { id: params.tenantId },
      data: { status: "ACTIVE", trialEndsAt: null, suspendedAt: null, planId: license.planId ?? undefined },
    }),
  ]);
...
export async function listLicenses(params: { q?: string | null; page?: number; pageSize?: number }) {
  ...
  const where = q ? { OR: [{ serial: { contains: normalizeSerial(q) } }, { tenant: { name: { contains: q } } }, { tenant: { slug: { contains: q } } }] } : {};

  const [total, items] = await Promise.all([
    prisma.licenseKey.count({ where }),
    prisma.licenseKey.findMany({
      where, orderBy: { createdAt: "desc" }, skip: (page - 1) * pageSize, take: pageSize,
      select: { id: true, serial: true, createdAt: true, redeemedAt: true, revokedAt: true, expiresAt: true, plan: { select: { slug: true, name: true } }, tenant: { select: { id: true, name: true, slug: true } } },
    }),
  ]);
...
export async function generateLicenses(params: { planSlug: string; qty: number; expiresAt?: Date | null }) {
  ...
  for (let i = 0; i < params.qty; i++) {
    const row = await prisma.licenseKey.create({
      data: { serial: normalizeSerial(generateSerial("PPOS")), planId: plan.id, expiresAt: params.expiresAt ?? null },
      select: { serial: true },
    });
    created.push(row);
  }
