...
import { invalidatePermissionCache } from "@/shared/server/cache/index";
...
export async function updateRolePermissions({ tenantId, input }: { tenantId: string; input: UpdateRolePermissionsInput }) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (filtered.length > 0) {
      await tx.rolePermission.createMany({ data: filtered.map((permissionId) => ({ roleId: input.roleId, permissionId })), skipDuplicates: true });
    }
  });
