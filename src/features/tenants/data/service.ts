...
export async function createTenantForExistingUser({ userId, tenantName, planSlug }: { userId: string; tenantName: string | null; planSlug?: string | null }) {
  ...
  const derivedTenantName = (tenantName && tenantName.trim()) || (user.name && user.name.trim().length >= 2 ? `Bisnis ${user.name.trim()}` : null) || (user.email ? `Bisnis ${user.email.split("@")[0]}` : null) || "Bisnis Baru";
  ...
  async function createTenantWithSlug(slug: string) {
    return prisma.tenant.create({
      data: { name: derivedTenantName, slug, planId: plan?.id ?? null, status: trialDays > 0 ? "TRIAL" : "ACTIVE", trialEndsAt },
      select: { id: true },
    });
  }
  ...
  await prisma.$transaction([
    ...
    prisma.branch.create({ data: { id: branchId, tenantId: createdTenant.id, code: "MAIN", name: "Main Branch", isActive: true } }),
    prisma.tenantUser.create({ data: { tenantId: createdTenant.id, userId, roleId: ownerRoleId, branchId } }),
  ]);
