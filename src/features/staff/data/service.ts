...
export async function listStaff(params: { tenantId: string; q?: string | null }) {
  ...
  const where = {
    ...
    ...(q ? { OR: [{ user: { name: { contains: q } } }, { user: { email: { contains: q } } }, { role: { name: { contains: q } } }] } : {}),
  };
...
export async function upsertStaff(params: { tenantId: string; input: UpsertStaffInput }) {
  ...
  if (params.input.id) {
    ...
    const updates = await prisma.$transaction(async (tx) => {
      const userUpdate = await tx.user.update({
        ...
        data: { name: params.input.name, phone, email, ...(password ? { passwordHash: await bcrypt.hash(password, 12), emailVerified: null } : {}) },
        select: { id: true, email: true, name: true, emailVerified: true },
      });

      const memUpdate = await tx.tenantUser.update({
        where: { id: membership.id },
        data: { roleId: role.id, branchId },
        select: { id: true },
      });
  ...
  const existingUser = await prisma.user.findUnique({ where: { email }, select: { id: true, passwordHash: true, accounts: { select: { provider: true } } } }).catch(() => null);
  if (existingUser) {
    const existsMembership = await prisma.tenantUser.findFirst({ where: { tenantId: params.tenantId, userId: existingUser.id }, select: { id: true } });
  ...
  const created = await prisma.$transaction(async (tx) => {
    const user = existingUser
      ...
      : await tx.user.create({
          data: { name: params.input.name, email, phone, passwordHash: password ? await bcrypt.hash(password, 12) : null, emailVerified: null },
          select: { id: true, email: true, name: true, emailVerified: true },
        });

    const membership = await tx.tenantUser.create({
      data: { tenantId: params.tenantId, userId: user.id, roleId: role.id, branchId },
      select: { id: true },
    });
