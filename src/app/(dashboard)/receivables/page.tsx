...
import { getReceivableOverview, listReceivables } from "@/features/receivables/data/service";
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={<HandCoins className="h-5 w-5" />} title="Total Piutang" value={fmt(overview.totalReceivable)} />
        <StatCard icon={<Clock className="h-5 w-5" />} title="Sisa Tagihan" value={fmt(overview.totalRemaining)} tone="slate" />
      ...
      <Card className="rounded-2xl">
        <CardContent className="flex items-center justify-between py-4 text-sm text-muted-foreground">
          <div>Menampilkan {result.items.length} dari {result.total} piutang • Page {result.page}/{totalPages}</div>
          <div className="flex gap-2">
            <Button asChild variant="outline" size="sm" className="rounded-xl" disabled={result.page <= 1}><Link href={`/receivables?${qs(prevPage)}`}>Prev</Link></Button>
            <Button asChild variant="outline" size="sm" className="rounded-xl" disabled={result.page >= totalPages}><Link href={`/receivables?${qs(nextPage)}`}>Next</Link></Button>
          </div>
        </CardContent>
      </Card>
    </div>
