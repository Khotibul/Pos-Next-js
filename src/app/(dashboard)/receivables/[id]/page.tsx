...
import { getReceivableById } from "@/features/receivables/data/service";
...
export default async function ReceivableDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    data = await getReceivableById({ tenantId: ctx.tenantId, id });
  } catch {
    notFound();
  }
  ...
  return (
    <div className="grid gap-4">
      <div className="flex items-center gap-2">
        <Button asChild variant="outline" size="sm" className="rounded-xl"><Link href="/receivables"><ArrowLeft className="h-4 w-4 mr-1" /> Kembali</Link></Button>
      </div>
      <PageHeader title={`Piutang ${data.invoiceNo}`} description={data.description ?? `Pelanggan: ${data.customerName ?? "-"}`} />
      ...
      <Card className="rounded-2xl">
        <CardHeader><CardTitle className="text-base">Info Piutang</CardTitle></CardHeader>
        <CardContent className="grid gap-2 text-sm">
          ...
          <div className="flex justify-between"><span className="text-muted-foreground">Pelanggan</span><span>{data.customerName ?? "-"}</span></div>
          ...
          <div className="flex justify-between"><span className="text-muted-foreground">Sale ID</span><span className="font-mono text-xs">{data.saleId ?? "-"}</span></div>
