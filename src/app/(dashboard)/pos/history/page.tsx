...
export default async function TransactionHistoryPage({
  ...
  const result = await listSales({ tenantId: ctx.tenantId, q, status, page: Number.isFinite(page) ? page : 1, pageSize: 20 });
  ...
  const chipHref = (nextStatus: string | null) => {
    ...
    params.set("page", "1");
    return `/pos/history?${params.toString()}`;
  };
  ...
  return (
    <div className="grid gap-4">
      <PageHeader
        title="Riwayat"
        description="Pantau transaksi terbaru Anda secara real-time."
        actions={
      ...
      <div className="md:hidden">
        <Card className="rounded-3xl">
          <CardContent className="grid gap-3 p-4">
            <form className="relative" action="/pos/history">
        ...
        <div className="mt-4 grid gap-2 rounded-2xl border bg-background p-3 text-xs text-muted-foreground">
          <div>
            Menampilkan {result.items.length} dari {result.total} transaksi • Page {result.page}/{totalPages}
          </div>
          <div className="flex gap-2">
            <Button asChild variant="outline" size="sm" className="flex-1 rounded-xl" disabled={result.page <= 1}>
              <Link
                href={`/pos/history?${new URLSearchParams({
                  ...(q ? { q } : {}),
                  ...(status ? { status } : {}),
          ...
          <CardContent className="flex items-center justify-between py-4 text-sm text-muted-foreground">
            ...
            <div className="flex gap-2">
              <Button asChild variant="outline" size="sm" className="rounded-xl" disabled={result.page <= 1}>
                <Link href={`/pos/history?${new URLSearchParams({ ...(q ? { q } : {}), ...(status ? { status } : {}), page: String(Math.max(1, result.page - 1)) }).toString()}`}>
                  Prev
                </Link>
              </Button>
              <Button asChild variant="outline" size="sm" className="rounded-xl" disabled={result.page >= totalPages}>
                <Link href={`/pos/history?${new URLSearchParams({ ...(q ? { q } : {}), ...(status ? { status } : {}), page: String(Math.min(totalPages, result.page + 1)) }).toString()}`}>
                  Next
                </Link>
              </Button>
            </div>
