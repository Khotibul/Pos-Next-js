...
function planFeatures(slug: string) {
  ...
  if (slug === "pro") {
    return [
      ...
      "Analitik & Prediksi Penjualan",
      "Integrasi E-wallet & QRIS",
      "Hingga 5 Outlet & Multi-kasir",
    ];
  }
  if (slug === "enterprise") {
        ...
        <CardContent className="grid gap-6 p-7">
          ...
          {isCustom ? (
            <Button
              asChild
              variant="outline"
              className="h-11 rounded-xl bg-[hsl(var(--sidebar))] text-white hover:bg-black/90 hover:text-white"
            >
    ...
    <main className="mx-auto max-w-6xl px-4 pb-28 pt-8 sm:px-6 sm:pt-12">
      ...
      <section className="mt-16 grid items-center gap-6 rounded-[36px] border bg-background/70 p-6 shadow-sm backdrop-blur lg:grid-cols-2 lg:p-8">
        ...
        <div className="overflow-hidden rounded-3xl border bg-background">
          <Image src="/landing-dashboard.svg" alt="Security infrastructure" width={1200} height={800} className="h-[220px] w-full object-cover opacity-90" />
        </div>
      </section>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 text-sm text-muted-foreground">
          ...
          <div className="flex flex-wrap gap-6 text-xs">
            {["Privacy Policy", "Terms of Service", "Contact Us", "Documentation"].map((x) => (
              <Link key={x} href="/about" className="hover:text-foreground">
                {x}
              </Link>
            ))}
          </div>
