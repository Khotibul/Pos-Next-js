...
function getAutoTableFn(mod: unknown): AutoTableFn {
  const candidate = isRecord(mod) && typeof mod.default === "function" ? mod.default : mod;
  if (typeof candidate !== "function") {
    throw new Error("Library PDF gagal dimuat. Silakan coba export CSV/Excel.");
  }
...
export async function GET(req: Request) {
  try {
    ...
    const autoTable = getAutoTableFn(await import("jspdf-autotable"));
    ...
    const message = error instanceof Error ? error.message : "Gagal export produk PDF.";
