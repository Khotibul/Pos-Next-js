...
export async function GET(request: Request) {
  const limit = await checkRateLimit("download", `${getClientIp(request)}:android`);
  if (!limit.success) return NextResponse.json({ ok: false, message: "Terlalu banyak request download." }, { status: 429 });

  const r = getLatestRelease("ANDROID");
  if (!r) return NextResponse.json({ ok: false, message: "Rilis Android belum tersedia." }, { status: 404 });
