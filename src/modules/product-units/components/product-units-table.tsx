...
import { upsertProductUnitAction, deleteProductUnitAction } from "@/modules/product-units/actions";
...
export function ProductUnitsTable({ items, q }: { items: Item[]; q?: string | null }) {
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Item | null>(null);
  const [notice, setNotice] = useState<string | null>(null);

  const [state, formAction, isPending] = useActionState(upsertProductUnitAction, null);
    ...
    <div className="grid gap-4">
            ...
            {items.length === 0 ? (
              ...
              items.map((u) => (
                <TableRow key={u.id}>
                  ...
                  <TableCell className="text-muted-foreground">{new Date(u.updatedAt).toLocaleString("id-ID")}</TableCell>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl rounded-2xl" key={editing?.id ?? "new"}>
          <DialogHeader>
            <DialogTitle>{editing ? "Ubah Satuan" : "Tambah Satuan"}</DialogTitle>
            <DialogDescription>Data satuan tersimpan per-tenant dan tercatat audit log.</DialogDescription>
          </DialogHeader>
          ...
          <form action={formAction} className="mt-3 grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input id="name" name="name" defaultValue={editing?.name ?? ""} placeholder="Contoh: pcs, box, kg" />
      ...
      <Dialog open={Boolean(confirm)} onOpenChange={(v) => (!v ? setConfirm(null) : null)}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{confirm ? `Satuan "${confirm.name}" akan dihapus. Produk yang memakai satuan ini akan menjadi tanpa satuan.` : null}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setNotice(null);
                startDelete(async () => {
                  const res = await deleteProductUnitAction(confirm.id);
                  if (!res.ok) {
