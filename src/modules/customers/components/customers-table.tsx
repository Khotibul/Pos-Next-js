...
import { upsertCustomerAction, deleteCustomerAction } from "@/modules/customers/actions";
...
export function CustomersTable({
  ...
  const [state, formAction, isPending] = useActionState(upsertCustomerAction, null);
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl rounded-2xl" key={editing?.id ?? "new"}>
          <DialogHeader>
            ...
            <DialogDescription>Data pelanggan tersimpan per-tenant dan tercatat audit log.</DialogDescription>
          </DialogHeader>
          ...
          <form action={formAction} className="mt-3 grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input id="name" name="name" defaultValue={editing?.name ?? ""} placeholder="Nama pelanggan" />
            ...
            <div className="grid gap-2">
              ...
              <Input id="address" name="address" defaultValue={editing?.address ?? ""} placeholder="Alamat pelanggan (opsional)" />
      ...
      <Dialog open={Boolean(confirm)} onOpenChange={(v) => (!v ? setConfirm(null) : null)}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{confirm ? `Pelanggan "${confirm.name}" akan dihapus permanen.` : null}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setNotice(null);
                startDelete(async () => {
                  const res = await deleteCustomerAction(confirm.id);
                  if (!res.ok) {
