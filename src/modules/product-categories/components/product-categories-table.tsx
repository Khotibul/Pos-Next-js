...
import { upsertProductCategoryAction, deleteProductCategoryAction } from "@/modules/product-categories/actions";
...
export function ProductCategoriesTable({ items, q }: { items: Item[]; q?: string | null }) {
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Item | null>(null);
  const [notice, setNotice] = useState<string | null>(null);

  const [state, formAction, isPending] = useActionState(upsertProductCategoryAction, null);
        ...
        <DialogContent className="max-w-xl rounded-2xl" key={editing?.id ?? "new"}>
          ...
          <form action={formAction} className="mt-3 grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input id="name" name="name" defaultValue={editing?.name ?? ""} placeholder="Contoh: Minuman" />
      ...
      <Dialog open={Boolean(confirm)} onOpenChange={(v) => (!v ? setConfirm(null) : null)}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            ...
            <DialogDescription>{confirm ? `Kategori "${confirm.name}" akan dihapus. Produk yang memakai kategori ini akan menjadi tanpa kategori.` : null}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                ...
                setNotice(null);
                startDelete(async () => {
                  const res = await deleteProductCategoryAction(confirm.id);
                  if (!res.ok) {
