...
export async function upsertProductDiscountAction(_prev: unknown, formData: FormData): Promise<ActionResult<{ id: string }>> {
  try {
    await requirePermission(PERMISSIONS.products_discount_manage);
    const ctx = await requireActiveTenant();

    const parsed = upsertProductDiscountSchema.safeParse(formDataToObject(formData));
    ...
    const res = await upsertProductDiscount({ tenantId: ctx.tenantId, input: parsed.data });
...
export async function deleteProductDiscountAction(id: string): Promise<ActionResult<{ id: string }>> {
  try {
    await requirePermission(PERMISSIONS.products_discount_manage);
    const ctx = await requireActiveTenant();

    const res = await deleteProductDiscount({ tenantId: ctx.tenantId, id });
    await writeAuditLog({ tenantId: ctx.tenantId, userId: ctx.userId, action: "DELETE", entity: "ProductDiscount", entityId: res.id });
