...
import { upsertPayableAction, deletePayableAction, createPayablePaymentAction } from "@/features/payables/actions";
...
export function PayablesTable({ items, q, status, suppliers }: { items: Item[]; q?: string | null; status?: string | null; suppliers: SupplierOpt[] }) {
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<Item | null>(null);
  const [notice, setNotice] = useState<string | null>(null);
  const [payOpen, setPayOpen] = useState<Item | null>(null);

  const [state, formAction, isPending] = useActionState(upsertPayableAction, null);
  const [payState, payAction, isPayPending] = useActionState(createPayablePaymentAction, null);
  ...
  useEffect(() => { if (state && state.ok) { setOpen(false); setEditing(null); } }, [state]);
  useEffect(() => { if (payState && payState.ok) { setPayOpen(null); } }, [payState]);
    ...
    <div className="grid gap-4">
      ...
      <Card className="rounded-2xl">
        <CardContent className="py-4">
          <form className="flex flex-wrap gap-2">
            <input name="q" defaultValue={q ?? ""} placeholder="Cari invoice / supplier..." className="h-11 w-full max-w-md rounded-xl border bg-background px-3 text-sm" />
            ...
            <Button type="button" className="ml-auto rounded-xl" onClick={() => { setNotice(null); setEditing(null); setOpen(true); }}>Tambah Utang</Button>
          </form>
        </CardContent>
      </Card>
      ...
      <div className="overflow-x-auto rounded-2xl border bg-background">
        <Table>
          ...
          <TableBody>
            {items.length === 0 ? (
              <TableRow><TableCell colSpan={8} className="py-10 text-center text-muted-foreground">Belum ada utang.</TableCell></TableRow>
            ) : items.map((r) => (
              <TableRow key={r.id}>
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-1.5">
                    <Button asChild variant="outline" size="sm" className="h-8 w-8 rounded-xl p-0"><Link href={`/payables/${r.id}`}><Eye className="h-3.5 w-3.5" /></Link></Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-xl rounded-2xl" key={editing?.id ?? "new"}>
          <DialogHeader>
            <DialogTitle>{editing ? "Ubah Utang" : "Tambah Utang"}</DialogTitle>
            <DialogDescription>Catat utang ke supplier (hutang toko ke supplier). Bisa dari pembelian kredit atau manual.</DialogDescription>
          </DialogHeader>
          ...
          <form action={formAction} className="mt-3 grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input id="invoiceNo" name="invoiceNo" defaultValue={editing?.invoiceNo ?? ""} placeholder="INV-UTANG-001" required />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="supplierId">Supplier</Label>
              <select id="supplierId" name="supplierId" defaultValue={editing?.supplierId ?? ""} className="h-11 rounded-xl border bg-background px-3 text-sm">
            ...
            <div className="grid gap-2">
              ...
              <Input id="description" name="description" defaultValue={editing?.description ?? ""} placeholder="Utang pembelian kredit..." />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="grid gap-2">
                <Label htmlFor="totalAmount">Total Utang (Rp)</Label>
      ...
      <Dialog open={Boolean(payOpen)} onOpenChange={(v) => !v && setPayOpen(null)}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle>Bayar Utang — {payOpen?.invoiceNo}</DialogTitle>
          ...
          <form action={payAction} className="grid gap-4">
            <input type="hidden" name="payableId" value={payOpen?.id ?? ""} />
            <div className="grid gap-2">
              <Label htmlFor="pay-amount2">Jumlah Bayar</Label>
              <Input id="pay-amount2" name="amount" type="number" min={1} max={payOpen?.remainingAmount ?? undefined} placeholder="0" required />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="pay-method2">Metode</Label>
              <select id="pay-method2" name="method" defaultValue="CASH" className="h-11 rounded-xl border bg-background px-3 text-sm">
            ...
            <div className="grid gap-2">
              <Label htmlFor="pay-reference2">Referensi</Label>
              <Input id="pay-reference2" name="reference" placeholder="No. referensi / opsional" />
            </div>
            <div className="grid gap-2">
              <Label htmlFor="pay-notes2">Catatan</Label>
              <Input id="pay-notes2" name="notes" placeholder="Catatan pembayaran" />
            </div>
            <DialogFooter>
      ...
      <Dialog open={Boolean(confirm)} onOpenChange={(v) => !v && setConfirm(null)}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle>Hapus utang?</DialogTitle>
          ...
          <DialogFooter>
            ...
            <Button className="rounded-xl" type="button" disabled={isDeleting || !confirm} onClick={() => {
              if (!confirm) return;
              startDelete(async () => {
                const res = await deletePayableAction(confirm.id);
