...
export const CACHE_TTL = {
  ...
  authUser: Number(process.env.REDIS_CACHE_TTL_AUTH_USER ?? 600),
  tenantContext: Number(process.env.REDIS_CACHE_TTL_TENANT_CONTEXT ?? 300),
  sidebar: Number(process.env.REDIS_CACHE_TTL_SIDEBAR ?? 600),
  permissions: Number(process.env.REDIS_CACHE_TTL_PERMISSIONS ?? 300),
  emailVerified: Number(process.env.REDIS_CACHE_TTL_EMAIL_VERIFIED ?? 600),
...
export const cacheKeys = {
  ...
  emailVerified: (userId: string) => `auth:email-verified:${userId}`,
